...
class ActionSequence(BaseModel):
    ...
    def validate_actions(v):
        ...
        if any(len(a) != 34 for a in v):
            raise ValueError(f"Expected 34 action dimensions, got mixed")
        return v
...
class ObservationSequence(BaseModel):
    ...
    def validate_obs(v):
        ...
        if any(len(o) != 12 for o in v):
            raise ValueError(f"Expected 12 observation dimensions, got mixed")
        return v
...
class ProductionRoboticsInferenceEngine:
    ...
    def __init__(self, model_path='./models/best.pt', 
        ...
        logger.info(f"✅ Engine Loaded: {self.full_action_dim}-dim action space, {self.obs_dim}-dim observations")
